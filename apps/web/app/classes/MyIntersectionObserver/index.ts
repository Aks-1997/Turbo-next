...
export type Callback = (entries: IntersectionObserverEntry[], observer: IntersectionObserver) => void;
...
class MyIntersectionObserver {
  ...
  initialiseInstance(callback: Callback, options: Options) {
    if (!this.instance) IOInstance = new IntersectionObserver(callback, options);
